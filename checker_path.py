...
def solution(B):  
    ...
    for node in search.checked:
        ...
        result = 0
        while True:
            xy = str(node.oRow) + str(node.oCol)
            path.append(xy)
    ...
    for path in visualisation:
        if len(path) != max(results):
            visualisation = visualisation[1:]

    # Visualise the game
    showBoard(B, visualisation)
...
def showBoard(board, path):
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            ...
            # Draw the position of tha pawns initially
            if board[i][j] == 'X':
                t.color('#a10000')
                t.begin_fill()
                t.circle(50)
